...
layui.define(['laypage', 'form'], function (exports) {
    ...
    IconPicker.prototype.render = function(options){
        var opts = options,
            ...
            type = opts.type ? opts.type : 'fontClass',
            // 是否分页：true/false
            page = opts.page,
            // 每页显示数量
            limit = opts.limit ? opts.limit : 12,
            // 是否开启搜索：true/false
            search = opts.search ? opts.search : true,
            // 点击回调
            click = opts.click,
            //
            select = opts.select ? opts.select : false,
            // json数据
            data = {},
            // 唯一标识
            ...
            createSelect: function () {
                var selectHtml = '<div class="layui-iconpicker layui-unselect layui-form-select" id="'+ ICON_BODY +'">' +
                    '<div class="'+ TITLE +'" id="'+ TITLE_ID +'">' +
                        '<div id="lay-icons">'+
                            '<div class="layui-iconpicker-icon layui-unselect">' +
                ...
                $(function(){
                    if($(elem).val() != ''){
                        $('#' + TITLE_ID).find('#lay-icons').find('.layui-iconpicker-icon').first().hide();
                        $.each($(elem).val().split(','),function(index,item){
                            $('#' + TITLE_ID).find('#lay-icons').append('<div class="layui-iconpicker-icon layui-unselect" ondblclick="var icons = $(\'#lay-icons\').parent().parent().prev().val().split(\',\');var a = $(this).find(\'i\').first().attr(\'class\');for (var i = 0; i < icons.length; i++) {if (icons[i] == a) {icons.splice(i, 1);break;}};$(\'#lay-icons\').parent().parent().prev().attr(\'value\',icons);if(icons == \'\'){$(\'#lay-icons\').find(\'.layui-iconpicker-icon\').first().show()};$(this).remove();" title="双击删除" alt="双击删除"><i class="'+item+'"></i></div>');
                        })
                    }
                })
                return a;
            ...
            check: function () {
                ...
                a.event('click', item, function (e) {
                    ...
                    if (isFontClass) {
                        ...
                        if(select == true){
                            //多选
                            $('#' + TITLE_ID).find('#lay-icons').append('<div class="layui-iconpicker-icon layui-unselect" ondblclick="var icons = $(\'#lay-icons\').parent().parent().prev().val().split(\',\');var a = $(this).find(\'i\').first().attr(\'class\');for (var i = 0; i < icons.length; i++) {if (icons[i] == a) {icons.splice(i, 1);break;}};$(\'#lay-icons\').parent().parent().prev().attr(\'value\',icons);if(icons == \'\'){$(\'#lay-icons\').find(\'.layui-iconpicker-icon\').first().show()};$(this).remove();" title="双击删除" alt="双击删除"><i class="'+icon+'"></i></div>');
                        }else{
                            //单选
                            $('#' + TITLE_ID).find('#lay-icons').html('<div class="layui-iconpicker-icon layui-unselect"><i class="'+icon+'"></i></div>');
                        }
                    } else {
                        var cls = $(e.currentTarget).find('.layui-icon').html(),
                            icon = cls;
                        if(select == true){
                            //多选
                            $('#' + TITLE_ID).find('#lay-icons').append('<div class="layui-iconpicker-icon layui-unselect" ondblclick="var icons = $(\'#lay-icons\').parent().parent().prev().val().split(\',\');var a = $(this).find(\'i\').first().html();for (var i = 0; i < icons.length; i++) {if (icons[i] == a) {icons.splice(i, 1);break;}};$(\'#lay-icons\').parent().parent().prev().attr(\'value\',icons);if(icons == \'\'){$(\'#lay-icons\').find(\'.layui-iconpicker-icon\').first().show()};$(this).remove();" title="双击删除" alt="双击删除"><i class="layui-icon">'+icon+'</i></div>');
                        }else{
                            //单选
                            $('#' + TITLE_ID).find('#lay-icons').html('<div class="layui-iconpicker-icon layui-unselect"><i class="layui-icon">' + icon + '</i></div>');
                        }
                    }

                    //选中的值赋值给input
                    $('#' + ICON_BODY).removeClass(selected).addClass(unselect);
                    if($(elem).val() == '' || select == false ){
                        $(elem).attr('value', icon);
                    }else if($(elem).val() != '' && select == true){
                        $(elem).attr('value', $(elem).val() + ',' + icon);
                    }
                    // 回调
                    if (click) {
                        click({
        ...
        var common = {
             ...
             */
            loadCss: function () {
                var css = '.layui-iconpicker {min-width: 289px;}.layui-iconpicker .layui-anim{display:none;position:absolute;left:0;top:42px;padding:5px 0;z-index:899;min-width:320px;border:1px solid #d2d2d2;max-height:300px;overflow-y:auto;background-color:#fff;border-radius:2px;box-shadow:0 2px 4px rgba(0,0,0,.12);box-sizing:border-box;}.layui-iconpicker-item{border:1px solid #e6e6e6;width:50px;height:38px;border-radius:0px 4px 4px 0px;cursor:pointer;position:relative;float:left}.layui-iconpicker-icon{border-left:1px solid #e6e6e6;border-top:1px solid #e6e6e6;border-bottom:1px solid #e6e6e6;-webkit-box-sizing:border-box;-moz-box-sizing:border-box;box-sizing:border-box;display:block;min-width:60px;height:40px;float:left;text-align:center;background:#fff;transition:all .3s;cursor:pointer;}.layui-iconpicker-icon i{line-height:38px;font-size:18px;}.layui-iconpicker-item > .layui-edge{margin-right:10px;}.layui-iconpicker-item:hover{border-color:#D2D2D2!important;}.layui-iconpicker-item:hover .layui-iconpicker-icon{border-color:#D2D2D2!important;}.layui-iconpicker.layui-form-selected .layui-anim{display:block;}.layui-iconpicker-body{padding:6px;width:320px}.layui-iconpicker .layui-iconpicker-list{background-color:#fff;border:1px solid #ccc;border-radius:4px;}.layui-iconpicker .layui-iconpicker-icon-item{display:inline-block;width:42px;line-height:36px;text-align:center;cursor:pointer;vertical-align:top;height:36px;margin:4px;border:1px solid #ddd;border-radius:2px;transition:300ms;font-size:15px;}.layui-iconpicker .layui-iconpicker-icon-item i.layui-icon{font-size:17px;}.layui-iconpicker .layui-iconpicker-icon-item:hover{background-color:#eee;border-color:#ccc;-webkit-box-shadow:0 0 2px #aaa,0 0 2px #fff inset;-moz-box-shadow:0 0 2px #aaa,0 0 2px #fff inset;box-shadow:0 0 2px #aaa,0 0 2px #fff inset;text-shadow:0 0 1px #fff;}.layui-iconpicker-search{position:relative;margin:0 0 6px 0;border:1px solid #e6e6e6;border-radius:2px;transition:300ms;}.layui-iconpicker-search:hover{border-color:#D2D2D2!important;}.layui-iconpicker-search .layui-input{cursor:text;display:inline-block;width:86%;border:none;padding-right:0;margin-top:1px;}.layui-iconpicker-search .layui-icon{position:absolute;top:11px;right:4%;}.layui-iconpicker-tips{text-align:center;padding:8px 0;cursor:not-allowed;}.layui-iconpicker-page{margin-top:6px;margin-bottom:-6px;font-size:12px;padding:0 2px;}.layui-iconpicker-page-count{display:inline-block;}.layui-iconpicker-page-operate{display:inline-block;float:right;cursor:default;}.layui-iconpicker-page-operate .layui-icon{font-size:12px;cursor:pointer;}.layui-iconpicker-body-page .layui-iconpicker-icon-limit{display:none;}.layui-iconpicker-body-page .layui-iconpicker-icon-limit:first-child{display:block;}.close-icon{width: 13px !important;height: 13px;background: rgba(0,0,0,0.3);line-height: 9px;position: relative;left: 10px;top: -17px;border-radius: 0px 0px 0px 10px;display: inline-block;cursor:pointer;padding:0px;}';
